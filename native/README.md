...
`core` contains deterministic CPU reference implementations and `shaderbench` provides a host-independent visual test loop. Slang kernels live in `shaders` and are the source for the eventual `prgpu` dispatch path.

...
`core` contains deterministic CPU reference implementations and `shaderbench` provides a host-independent visual test loop. Frame Gate, RGB Shift, Halftone, Dot Matrix, Dither, and Progressive Blur have reference implementations; the pixel effects also have Slang kernels in `shaders`.

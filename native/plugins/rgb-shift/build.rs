...
fn main() {
    // Premiere's GPU filter registration is an additional entry point beside
    // the AE-style software effect entry point.
    println!("cargo:rustc-cfg=with_premiere");
    println!("cargo:rustc-cfg=catch_panics");
    prgpu_build::effect()
        // prgpu-build 0.2.0 does not locate the vekl snapshot bundled in its
        // sibling prgpu crate when consumed from crates.io. Keep this narrow
        // resolver until a compatible upstream release fixes that lookup.
...
fn find_prgpu_vekl() -> PathBuf {
    ...
    let registries = std::fs::read_dir(&registry_sources).unwrap_or_else(|error| {
        panic!(
